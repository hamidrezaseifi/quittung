...
public class RechnungAdapter extends AdapterBase<RechnungEntity, RechnungModel> {
	...
	public RechnungEntity toEntity(RechnungModel model) {
		...
		if(model.isNew()) {
			entity = new RechnungEntity(model.getCustomerId(), model.getNummer(), model.getRechnungCreate(), model.getLiferDate(),
					model.getRechnungType().getValue(), model.getStatus().getValue(), model.getUserId());
		} 
		else {
			entity = new RechnungEntity(model.getId(), model.getCustomerId(), model.getNummer(), model.getRechnungCreate(), model.getLiferDate(),
					model.getRechnungType().getValue(), model.getStatus().getValue(), model.getUserId(), model.getUpdated());
		}
	...
	public RechnungModel toModel(RechnungEntity entity) {
		RechnungModel model = new RechnungModel(entity.getId(), entity.getCustomerId(), entity.getNummer(), entity.getRechnungCreate(), 
				entity.getLiferDate(), entity.getRechnungType(), entity.getStatus(), entity.getUserId(), entity.getCreated(), entity.getUpdated());
